...
- [x] **F1 · Primeira pessoa vira o cliente OFICIAL (D-27, decisão do ideador)**: o raycaster (`prototipos/fps/`, ilha flutuante + dia/noite + água shader + árvores L-system) é publicado pelo Pages em `/fps/` via `site/scripts/build-fps.mjs` (mundo da batida atual inlined a cada deploy; pages.yml já redeploya a cada batida). Botão JOGAR do README → `/fps/`; mapa 2D segue na raiz como visão de cima + painéis (Mural/Comércio/Portais), com links cruzados entre os dois.
- [x] F2 · Dieta do repo (D-28): binários regeneráveis fora da main — previews `_8x`/contact sheets/mapa mock agora saem em `assets/preview/` (gitignorado; `assets/tools/` ajustados), screenshots de QA (site/qa, prototipos) removidos e ignorados — evidência vive nos branches de trabalho; ~5MB fora do checkout e do deploy.
