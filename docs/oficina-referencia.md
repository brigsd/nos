...
- **Pintar faces:** **Shift+clique** soma faces à seleção (como nos vértices; a última é a "ativa"). Com face(s) selecionada(s), o painel mostra um **seletor de cor** + alguns presets; escolher uma cor **pinta as faces selecionadas** — é a operação `pincel` (modo face). Desfazível, e pintar a cor que a face já tem não faz nada. Por ora é cor **chapada por face**; o pincel macio com degradê (e a textura pintável de verdade) vem no passo 11.
