...
## v2 — fatia ⚔️ Combate por turnos até a tela (2026-07-15, branch `claude/v2-combate`, em PR)
Re-implementação limpa sobre a main (a v2 congelada foi referência; os bugs dela morreram no caminho):
- **Motor:** `engine/combat.ts` — `resolveCombat` (até 5 turnos, jogador golpeia primeiro, 80%/75% de acerto, stats por facção), `applyXP` (level-up: +10 de teto, cura total) e `regenNative`. Comando `/atacar alvo`: corpo a corpo (1 tile), 10 de energia, seed por evento (`seed + "-combate-" + nº da issue`) ⇒ mesma issue, mesma luta, golpe a golpe. Vitória: XP por facção + espólio FIXO cunhado (nunca rouba o estoque do mercador); derrota: renasce em (30,30) curado, energia ≤ 20, metade do XP perdido; 5 turnos sem queda = impasse.
- **Decisão de design (diverge da v2 congelada, motivo técnico + lore):** Nativo derrotado NÃO é deletado — **desfalece** (hp 0), pula a própria behavior tree e regenera +5/batida no `tickNatives`. Deletar órfão os eventos antigos dele e quebraria os cross-checks do validador (mundo inválido para sempre). Campos novos: `Player.hp/maxHp/level/xp` e `Native.maxHp` (opcionais, retrocompatíveis via `getCombatStats`/`getNativeMaxHp`).
- **Evento `combat_resolved`** carrega o roteiro turno-a-turno (`actions`), outcome, XP, espólio e HP final dos dois lados — o campo do alvo chama `nativeId` de propósito, para o cross-check genérico do validador cobrir de graça. Checks semânticos novos: hp ≤ teto (jogador e Nativo). Anti-drift: cap de `actions` no schema = `MAX_COMBAT_ROUNDS*2`.
- **Segurança:** `getOwn` no alvo (`/atacar __proto__` = "não existe", o bug de travamento da v2 morto com teste); atacar desfalecido é recusado.
- **Tela:** painel HUD "Combate" (abre sozinho quando a luta é da batida atual; log pt-BR revelado linha a linha em sincronia) + efeitos no canvas (talho + número de dano flutuante por golpe, 900ms por ação) + barra de HP fina sob Nativos feridos + Nativo desfalecido desenhado esmaecido + linha no Mural ("@brigsd mediu forças com Cinza — ninguém cedeu"). Verificado e2e (tick canônico: longe/hostil/luta real) e em Chromium headless.
- 206→256 testes. Follow-up quando as 3 fatias estiverem na main: "Meu Nó" mostrar HP/nível; `/trocar`//`conversar` recusarem Nativo desfalecido.
