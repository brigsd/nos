...
- **`site/src/auth-ui.ts`**: painel HUD "Entrar com GitHub" — formulário PAT (link pré-preenchido de criação de token fine-grained via template URL do GitHub: `target_name=brigsd&repositories=nos&issues=write`, texto de segurança em pt-BR) quando `NOS_OAUTH_CLIENT_ID` vazio; UI de device flow (código + link + "aguardando confirmação…") pronta atrás da flag.
