...
4. **Em andamento:** leva 2 — T3 (tick, branch `claude/v1-t3-tick`) e T4 (site canvas + Pages, branch `claude/v1-t4-site`). Também foi criada a branch `claude/v1-t5-avatar` que integra T3 + T4 e implementa a movimentação local do avatar (T5, WASD + Toque/Pathfinding).
