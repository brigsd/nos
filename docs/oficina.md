...
11. `[~]` Modos livres do pincel: raio, dureza, degradê. Acrescenta, não substitui. **Inclui (transferido do passo 9, D-88) a base de textura pintável: atlas por face + a textura gerada ao abrir** — o pré-requisito do pincel macio; o modo "face" migra pra essa textura sem trocar o formato salvo (`['pincel',...]`). **11a FEITO (D-90):** a base — `adaptarV3` troca o swatch por um atlas por face (projeção em caixa PRIVADA por face, sem o furo do topo/fundo da caixa global), render BYTE-IDÊNTICO ao swatch, só o adaptador. **11b MOTOR FEITO (D-91):** a op `pincel` aprendeu `modo:'livre'` (dab `{f,a,b}` face-local em `f.tinta`, `raio`/`dureza`) e o `adaptarV3` rasteriza o dab radial macio na ilha; determinístico, tinta acompanha a face, e o modo 'face' segue BYTE-idêntico (compat estrutural). Ambas as mudanças no núcleo desde o passo 1 (só `motor/oficina.js`). **Falta o 11c:** a interface — pintar arrastando na superfície (raycasting → `{f,a,b}`), fecha o passo 11.
