...
- S5b `[ ]` **Amarrar no jogo + o A/B**: o `som.js` passa a poder tocar um evento exportado (ponte ADITIVA, jogo byte-idêntico quando off) e a renderizar os próprios sons offline; e o A/B mede cada preset/evento contra o som REAL do jogo — o fecho do ouvido.
