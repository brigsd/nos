...
| `rotaciona` | `eixo` (`'x'\|'y'\|'z'`), `graus` (PARAM), `sel?` (`{v:[ids]}` e/ou `{f:[ids]}`, default = a malha inteira), `pivo?` (`[x,y,z]`, default = centroide da seleção) | FEITO (P3 do playground, só núcleo). Gira a seleção em torno do PIVÔ (`p' = pivo + R_eixo(graus)·(p−pivo)`, a mesma convenção right-handed das matrizes de animação `mRotX/mRotY/mRotZ`). SIMPLES: só desloca posições (`st.V.set` in-place) — NUNCA cria vértice/face nem renumera (o oposto do `espelha` acima). Id inexistente na seleção GRITA, nunca corrompe. |
