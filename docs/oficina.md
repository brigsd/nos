...
| `inflate` | `id`, `contornoLado`, `contornoTopo` | Dois contornos 2D (lado e topo, ver Aba Desenho) viram volume 3D. Roteiro (P6 do playground) — o FORMATO do contorno já existe (P5, D-118: a lista de pontos `[x,y]` que o `loft` usa em `contorno` e o `gabarito.mjs` usa em `CONTORNOS`); falta só a op que INFLA dois deles num volume. |
