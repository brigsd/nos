...
- **`render.js` é jóia** (compartilhado com o jogo): mudança nele é **opt-in e no-op quando desligada** (somar zero, guardar com `?? padrão`), e verificada BYTE-idêntica com o recurso desligado (`porteiro`/`cmp` nas peças do jogo). Nunca uma mudança que altere a saída do jogo sem intenção.
...
O olho erra em normal, luz, alinhamento e geometria (D-65). Onde der, prove com número: `cmp` byte-a-byte (render sem regressão), forma canônica bit-a-bit (replay/determinismo), projeção pelo próprio motor comparada ao esperado (posição na tela). "Parece bom" não é prova; o número é.
